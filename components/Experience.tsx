...
const ExperienceComponent = ({ title, time, company, location, companyLink, catagory, last, current }) => {
	return (
		<div className="flex">
			<p className="pt-[7px] mr-4 w-44 hidden sm:block text-right text-md text-light">{time}</p>
			<div className="flex flex-col w-10 flex-shrink-0 items-center mr-4">
				<div>
					{current === true ?
						<div className="absolute w-10 h-10 border-[3px] rounded-full border-accent-1-light animate-ping"></div>
						: null}
					<div className="flex items-center justify-center w-10 h-10 border-[3px] rounded-full border-accent-1-light">
						{(() => {
							switch (catagory) {
								case "education":
			...
			<div className="pt-1 pb-8">
				<p className="text-lg font-bold text-plain">{title}</p>
				<p className="mb-2 text-plain">
